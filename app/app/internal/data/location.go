...
// GetLocationsRunning2 .
func (lr *LocationRepo) GetLocationsRunning2(ctx context.Context) (map[int64][]*biz.Location, error) {
	var location []*Location
	...
	for _, vLocation := range location {
		if _, ok := res[vLocation.UserId]; !ok {
			res[vLocation.UserId] = make([]*biz.Location, 0)
		}

		res[vLocation.UserId] = append(res[vLocation.UserId], &biz.Location{
			ID:           vLocation.ID,
			UserId:       vLocation.UserId,
			Status:       vLocation.Status,
			CurrentLevel: vLocation.CurrentLevel,
